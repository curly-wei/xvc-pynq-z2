...
# https://forums.xilinx.com/t5/Design-Entry/referenced-RTL-module-and-synthesis/td-p/863818?fbclid=IwAR2sWgR--5-2ztTpNzFUe0viDiV23-nCyT364FEAw-PXjCU7t8GfjQ4VfRQ
# and
...
# only can compile axi_jtage with 'locked ip mode'
...
if { [ file exists ${kPSPresetFile} ] == 1 } {
  ...
  error "ERROR: read-in files for ps preset: < ${kPSPresetFile} > fail. This file is necessary, please check again"
}
...
if { [ file exists ${kTopBDScriptFile} ] == 1 } {
  ...
  error "ERROR: read-in files for ps preset: < ${kTopBDScriptFile} > fail. This file is necessary, please check again"
}
...
set top_bd_wrapper_path ".gen/sources_1/bd/${kBDName}/hdl/${top_bd_wrapper_name}.v"
...
#write_checkpoint -force "${kOutputDir}/post_synth"
...
report_timing -sort_by group -max_paths 100 -path_type summary -file "${kOutputDir}/post_route_timing.rpt"
...
write_hw_platform -fixed -include_bit -force -verbose "${kOutputDir}/${kBDName}_top.xsa"
